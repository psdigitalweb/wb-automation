...
def compute_funnel_signals(
    ...
    for row in eligible:
        ...
        result.append({
            ...
            "card_clicks": row.get("card_clicks", 0),
            "funnel_ctr_percent": row.get("funnel_ctr_percent"),
            "active_days_with_impressions": row.get("active_days_with_impressions", 0),
            "quality_excluded_rows": row.get("quality_excluded_rows", 0),
            "ctr_sample_tier": row.get("ctr_sample_tier", "insufficient"),
            "ctr_quality_flags": row.get("ctr_quality_flags", []),
            "title": row.get("title"),
