#!/usr/bin/env python3
"""Run experimental semantic query clustering for one project/category scope."""
...
def _running_inside_container() -> bool:
    return os.getenv("ECOMCORE_SEMANTIC_CLUSTERING_IN_DOCKER") == "1" or Path("/.dockerenv").exists()
...
def _rerun_in_api_container(argv: list[str]) -> int:
    ...
    exec_command = [
        ...
        "-T",
        "-e",
        "ECOMCORE_SEMANTIC_CLUSTERING_IN_DOCKER=1",
        "api",
        "python",
    ...
    print(
        ...
        "Re-running semantic clustering in the api container...",
        file=sys.stderr,
    )
    try:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Run SEO semantic clustering experiment")
    ...
    parser.add_argument("--model-name", default=DEFAULT_SEMANTIC_MODEL_NAME, help="SentenceTransformer model name")
    parser.add_argument("--similarity-threshold", type=Decimal, default=Decimal("0.80"), help="Semantic similarity threshold")
    parser.add_argument("--min-community-size", type=int, default=2, help="Minimum semantic community size")
    parser.add_argument("--strategy", default=DEFAULT_GATING_STRATEGY, help="Semantic gating strategy")
    parser.add_argument("--top-limit", type=int, default=20, help="Number of top clusters in diagnostics")
    parser.add_argument("--samples-limit", type=int, default=20, help="Number of sample rows in diagnostics")
    parser.add_argument(
        "--pretty",
    ...
    try:
        result = run_semantic_clustering_experiment(
            session,
            project_id=args.project_id,
            category_id=args.category_id,
            bucket=args.bucket,
            model_name=args.model_name,
            similarity_threshold=Decimal(str(args.similarity_threshold)),
            min_community_size=max(2, int(args.min_community_size)),
            strategy=args.strategy,
            top_limit=max(1, int(args.top_limit)),
            samples_limit=max(1, int(args.samples_limit)),
        )
        if result.raw_semantic.total_input_queries == 0:
            print(
                json.dumps(
                    {
                        "ok": False,
                        "error": "No kept queries found for semantic clustering scope",
                        "project_id": args.project_id,
                        "category_id": args.category_id,
                        "bucket": args.bucket,
                    },
        ...
        print(json.dumps(result.to_debug_payload(), ensure_ascii=False, indent=2 if args.pretty else None))
        return 0
