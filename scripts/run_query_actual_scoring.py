#!/usr/bin/env python3
"""Run deterministic actual scoring for one project/category/SKU scope."""
...
def _running_inside_container() -> bool:
    return os.getenv("ECOMCORE_ACTUAL_SCORING_IN_DOCKER") == "1" or Path("/.dockerenv").exists()
...
def _rerun_in_worker_container(argv: list[str]) -> int:
    ...
    exec_command = [
        ...
        "-T",
        "-e",
        "ECOMCORE_ACTUAL_SCORING_IN_DOCKER=1",
        "worker",
        "python",
    ...
    print(
        ...
        "Re-running actual scoring in the worker container...",
        file=sys.stderr,
    )
    try:
...
def main() -> int:
    ...
    from app.services.seo.scoring.preparation import QueryScoringPreparationError

    parser = argparse.ArgumentParser(description="Run SEO actual scoring")
    ...
    parser.add_argument("--top-limit", type=int, default=20, help="Number of top/bottom scored clusters in diagnostics")
    parser.add_argument(
        "--pretty",
            ...
            json.dumps(
                {
                    ...
                    "top_scores": [item.to_dict() for item in result.diagnostics.top_clusters],
                    "bottom_scores": [item.to_dict() for item in result.diagnostics.bottom_clusters],
                },
                ensure_ascii=False,
