...
def _running_inside_container() -> bool:
    return os.getenv("ECOMCORE_HYBRID_IN_DOCKER") == "1" or Path("/.dockerenv").exists()
...
def _rerun_in_api_container(argv: list[str]) -> int:
    ...
    print(
        ...
        "Re-running hybrid annotation in the api container...",
        file=sys.stderr,
    )
    try:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Run SEO query hybrid annotation")
    ...
    parser.add_argument("--top-limit", type=int, default=20, help="Reserved for parity with other SEO scripts")
    ...
    try:
        result = run_query_hybrid_annotation(
            session,
        ...
        if result.diagnostics.total_queries_processed == 0:
            session.rollback()
            print(
                json.dumps(
                    {
            ...
            json.dumps(
                {
                    ...
                    "sample_inherited_queries": [row.to_dict() for row in result.annotated_queries if row.provenance == "cluster"][
                        : max(1, int(args.samples_limit))
                    ],
                    "sample_rejected_queries": [row.to_dict() for row in result.annotated_queries if row.provenance == "rejected"][
                        : max(1, int(args.samples_limit))
                    ],
                },
                ensure_ascii=False,
